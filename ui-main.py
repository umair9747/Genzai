...
with st.sidebar:
    st.markdown(f'<p style="font-size: 1.5rem; font-weight: 600; color: {THEME["primary"]};">⚙️ Configuration</p>', unsafe_allow_html=True)
    ...
    if targets_input:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Single visualization for device categories
                st.markdown("-----")
                ...
                for result in results["Results"]:
                    with st.expander(f"{CATEGORY_ICONS.get(result['category'], '🔍')} {result['Target']}", expanded=True):
